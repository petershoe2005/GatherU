...
const CheckoutForm: React.FC<CheckoutScreenProps> = ({ item, onBack, onSuccess }) => {
  ...
  const platformFee = +(itemPrice * PLATFORM_FEE_RATE).toFixed(2);
  const total = +(itemPrice + platformFee).toFixed(2);
  ...
  if (succeeded) {
    return (
      <div className="flex-1 bg-background-dark min-h-screen flex flex-col items-center justify-center px-6 text-center">
        <div className="w-20 h-20 rounded-full bg-success/20 flex items-center justify-center mb-6 animate-in zoom-in duration-500">
        ...
        <p className="text-slate-400 text-sm">Your order for <span className="text-primary font-bold">{item.title}</span> has been placed.</p>
    ...
    <div className="flex-1 bg-background-dark min-h-screen flex flex-col">
      ...
      <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-background-dark/95 backdrop-blur-xl border-t border-slate-700 p-4 z-40">
        <button
          ...
          className="w-full bg-primary hover:bg-primary-light text-white font-black py-4 rounded-xl shadow-xl shadow-primary/20 transition-all active:scale-[0.98] flex items-center justify-center gap-2 disabled:opacity-40 disabled:active:scale-100"
        >
          {loading ? (
